...
export type Snippet = { label: string; filename: string; lines: string[] };
export type Project = { id: "caroot" | "protein" | "commerce" | "workflow"; title: string[]; meta: string[]; year: string; statement: string; notes: string[]; wide?: boolean; code: Snippet };

export const projects: Project[] = [
  { id:"caroot", title:["CaRoot"], meta:["CALORIE MANAGEMENT","MOBILE APP"], year:"2026", statement:"A personal calorie and meal management app that uses AI to make everyday food logging easier.", notes:["Food photo","Barcode / label","Nutrition"], wide:true, code:{ label:"TypeScript", filename:"caroot.ts", lines:['const meal = await analyseFood(photo);',"","const nutrition = await estimateNutrition(meal);","await saveMeal(nutrition);"] } },
  { id:"protein", title:["PROTEIN","STRUCTURE","ANALYSIS"], meta:["UNIVERSITY RESEARCH","STRUCTURAL BIOLOGY"], year:"2025—2026", statement:"University research development for comparing protein structures through inter-carbon distances, with automated retrieval and processing of structural data from UniProt and PDB.", notes:["UniProt / PDB","Cα distances","Structure analysis"], code:{ label:"Python", filename:"structure_analysis.py", lines:["structures = fetch_pdb_structures(uniprot_id)","","coordinates = extract_ca_coordinates(structures)","distances = pairwise_distances(coordinates)","compare_structures(distances)"] } },
  { id:"commerce", title:["WEB /","COMMERCE","PROJECTS"], meta:["CLIENT WORK","WEB DEVELOPMENT"], year:"2025—2026", statement:"Web and commerce development for client projects, from information architecture and frontend implementation to CMS and checkout integration.", notes:["Next.js","CMS / API","Stripe"], code:{ label:"TypeScript", filename:"web.ts", lines:['const content = await cms.getList({ endpoint: "products" });',"","export const products = content.contents;"] } },
...
export const capabilities: string[] = ["TypeScript","Python","Next.js","React","React Native","Expo","FastAPI","Supabase","PostgreSQL","Docker","n8n"];
export type CapabilityGroup = { label:string; slug:string; items:string[] };
export const capabilityGroups: CapabilityGroup[] = [
  { label:"LANGUAGES", slug:"languages", items:["TypeScript","Python"] },
  { label:"WEB", slug:"web", items:["Next.js","React"] },
