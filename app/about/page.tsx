...
const timeline = [
  ...
  { year: "EARLY CAREER", title: "LETTERFAN / DRUMROLL", body: "Gained practical development experience through Letterfan and Drumroll before moving into independent client work.", bodyJa: "Letterfan、Drumrollでの実務経験を経て、個人でのクライアントワークへ活動領域を広げました。" },
    ...
    <>
      ...
      <main className="doc">
        <section className="scene page-intro">
          ...
          <Reveal>
            <h1 className="t-display page-title"><span className="line-mask"><span>ENGINEER,</span></span><span className="line-mask"><span className="t-faint">DESIGNER, BUILDER.</span></span></h1>
            <p className="t-body page-lede soft-in"><LocalizedText en="I work across product development, web engineering, research software and workflow automation — taking ideas from requirements and structure through design, implementation and operation." ja="プロダクト開発、Webエンジニアリング、研究用ソフトウェア、業務自動化まで横断し、要件整理からデザイン、実装、運用まで一貫して形にしています。" /></p>
          </Reveal>
        </section>
        ...
        <section className="scene work-body">
          <Reveal className="work-section">
            ...
            <div className="brief">
              ...
              <p className="t-body soft-in"><LocalizedText en="After gaining practical experience through Letterfan and Drumroll, I moved into freelance engineering. Today my work ranges from mobile products and corporate websites to commerce, backend integrations and automation. I prefer to stay involved across the full path from understanding the problem to shipping and maintaining the final system." ja="Letterfan、Drumrollでの実務経験を経て、現在はフリーランスエンジニアとして活動しています。モバイルプロダクト、コーポレートサイト、EC、バックエンド連携、自動化まで幅広く、課題の理解から公開後の運用まで一貫して関わることを大切にしています。" /></p>
          ...
          <Reveal className="work-section">
            ...
            <ol className="built t-body">{timeline.map((item, i) => <li key={item.title} className="soft-in"><span className="built-n t-mono">{String(i + 1).padStart(2, "0")}</span><span><span className="t-mono">{item.year}</span><br /><strong>{item.title}</strong><br /><LocalizedText en={item.body} ja={item.bodyJa} /></span></li>)}</ol>
          </Reveal>
